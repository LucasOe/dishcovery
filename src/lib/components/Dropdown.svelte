...
<div class="relative space-y-xs rounded-sm bg-gray-500">
	...
	>
		{#each entries as entry}
			<li class={twMerge("cursor-pointer text-xl font-semibold", entry == selectedEntry && "text-yellow")}>
				<button
					on:click={() => {
						selectedEntry = entry;
						menuOpen = false;
