...
<div class="h-full relative flex"
     use:pan={{delay: 0}} on:pan={handlePan}
     on:mouseup={handlePanEnd} on:touchend={handlePanEnd}
     bind:offsetHeight={cardH} bind:offsetWidth={cardW}>
    {#await fetchRecipes()}
        <p class=" relative flex justify-center items-center w-full">Loading...</p>
    {:then recipes}
        <Card recipe={recipes[1]} class="h-full w-full absolute flex-auto"/>
        <Card recipe={recipes[0]} class="h-full w-full absolute flex-auto" transformValue="{transformValue}" touchStarted="{touchStarted}" swipeDirection="{swipeDirection}"/>
    {:catch error}
